...
namespace TersoSolutions.Jetstream.SDK.Application.Model
{
    ...
    /// 
    /// Request object for Jetstream RemoveEvents endpoint.
    /// </summary>
    public class RemoveEventsRequest : JetstreamRequest
    {
        private const string _removeEventsCommand = "v1.5/application/?action=removeevents&accesskey={0}{1}{2}";
        ...
        internal override string BuildUri(string baseUri, string accesskey)
        {
            ...
            if (EventIds != null && EventIds.Any())
            {
                ...
                string idString = String.Join("_", EventIds.Select(ev => HttpUtility.UrlEncode(ev)).ToArray());
                usingEvents = String.Format("&eventids={0}", idString);
            }

            // build the uri
            return String.Concat(baseUri, String.Format(_removeEventsCommand, accesskey, usingBatch, usingEvents));
        }
    }
}
