...
{
    /// <summary>
    /// The base Event DTO that all events inherit. Handles EventId and EventTime assignment.
    /// </summary>
    ...
    /// <summary>
    /// This class is a customized JsonConverter from the Newtonsoft library. This allows us to dynamically deserialize events to the
    /// correct class, using reflection. Note that this is dependent on EventDto being in the same namespace as the other event DTOs.
    /// </summary>
    public class EventDtoConverter : JsonConverter
    {
        /// <summary>
        /// Custom method that gets called for each JSON object in the Events list in the EventsDto. This is so we can
        /// deserialize each JSON object into the correct child event, instead of them all being EventDto and having data truncated.
        /// </summary>
        ...
        private static EventDto Create(JObject jObject)
        {
            ...
            try
            {
                ...
                // If the full type isn't found, cast into an EventDto and have the AdditionalData field handle unknown fields
                if (fullType == null)
                {
                    return new EventDto();
                }
            ...
            {
                // Something went wrong in the reflection, most likely that the type wasn't found.
                throw new ArgumentException("Error in reflection deserialization of event | " + jObject.ToString(Formatting.None), e);
            }
        }

        /// <summary>
        /// Only allow Event DTOs as the "result" object
        /// </summary>
        ...
        /// <summary>
        /// When reading JSON, call our custom Create method for each event
        /// </summary>
